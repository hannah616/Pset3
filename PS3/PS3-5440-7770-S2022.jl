...
md"""
initial stoichiometric array --> add additional column (M X R+1)
add additional rate to strain it to be exactly mu
how to convert doubling time to become growth rate?
x = x0 exp(u*t)
2 = x/x0 = exp(u*t) --> ln(2)/t = u
"""
...
begin
	...
	stephist(saturation_data_set, bins = number_of_bins, normed = :true,label = "N = $(length)")
...
begin
	...
	c1 = 0.01*203 #Vmax = concentration * kinetic rate (1/s)
	c2 = 0.01*34.5
	c3 = 0.01*249
	c4 = 0.01*88.1
	c5 = 0.01 * 13.7
...
# ╔═╡ 4c697709-58d1-4613-9d89-bd8d32149ddd
let
...
begin
	...
	# CF = 0.0
	#mol/L * Volume per cell (L) * weight per cell (cell/g) * (g/gDW)
	d = 12.4
	V = (4/3)*3.141592*(d/2)^3*(1e-15)
	m = 1e-9
	f = 0.7
	CF = V/m*(1/(1-f))*1e6
